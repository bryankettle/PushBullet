...
class PushbulletAPI {
	public:
		...
		void PBnotify(String title, String body); // sends a push notification
		void PBnotify(String title, String body, String fileurl); //sends a push notification with an image, currently only jpeg is supported. the fileurl is the http address of the image to be sent  
		void PBconnect(); //connects to Pushbullet API
		String PBupload(String filename); // uploads a file to PushBullet. the output will be the file url
	private:
